...
def train(net, train_loader, optimizer, scheduler):
  ...
  net.train()
  loss_ema = 0.
  for i, (images, targets) in tqdm.tqdm(enumerate(train_loader)):
...
def main():
  ...
  if args.dataset == 'cifar10':
    train_data = datasets.CIFAR10(root=args.data_path, train=True, transform=train_transform, download=True)
    test_data = datasets.CIFAR10(root=args.data_path, train=False, transform=test_transform, download=True)
    base_c_path = os.path.join(args.data_path, 'cifar/CIFAR-10-C/')
    base_c_bar_path = os.path.join(args.data_path, 'cifar/CIFAR-10-C-Bar/')
    num_classes = 10
  else:
    train_data = torchvision.datasets.CIFAR100(root="/p/lustre1/trivedi1/vision_data",
            train=True,
            transform=train_transform)
    test_data = torchvision.datasets.CIFAR100(root="/p/lustre1/trivedi1/vision_data",
        train=False,
        transform=test_transform) 
    base_c_path = os.path.join(args.data_path, 'CIFAR-100-C/')
    base_c_bar_path = os.path.join(args.data_path, 'CIFAR-100-C-Bar/')
    num_classes = 100
  ...
  save_name =  args.dataset + '_' + args.model + '_'+ "pixmix" 
  log_path = os.path.join("/p/lustre1/trivedi1/compnets/playground/logs",
                            save_name + '_training_log.csv')
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    if epoch % 10 == 0:
        s = "{save_name}_checkpoint_{epoch:03d}_pth.tar".format(save_name=save_name,epoch=epoch)
        save_path = os.path.join(args.save, s)
        torch.save(checkpoint, save_path)
    
    if is_best:
        save_path = os.path.join(args.save, save_name + '_model_best.pth.tar')
        torch.save(checkpoint, save_path)
  ...
  Save Final Ckpt.
  """
  s = "{save_name}_final_checkpoint_{epoch:03d}_pth.tar".format(save_name=save_name,epoch=epoch)
  save_path = os.path.join(args.save, s)
  torch.save(checkpoint, save_path)
