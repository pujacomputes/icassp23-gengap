...
from core.get_data import create_dataset,create_dataset_good
    ...
    for run in range(cfg.train.runs):
        ...
        for epoch in range(cfg.train.epochs):
            ...
            print(f'Epoch: {epoch:03d}, Train perf: {train_perf:.4f}, Train Loss: {train_loss:.4f}, '
                  f'Val: {val_perf:.4f}, Test: {test_perf:.4f}, Seconds: {time_cur_epoch:.4f}',flush=True)
...
def run_good(cfg, create_dataset, create_model, train, test, evaluator=None):
    ...
    train_dataset, val_dataset, test_dataset = create_dataset(cfg)
    eval_train_dataset, _, _, ood_val_dataset, ood_test_dataset = create_dataset_good(cfg)
    train_loader = DataLoader(
        train_dataset, cfg.train.batch_size, shuffle=True, num_workers=cfg.num_workers)
    val_loader = DataLoader(
        val_dataset,  cfg.train.batch_size, shuffle=False, num_workers=cfg.num_workers)
    ...
    ood_val_loader = DataLoader(
        ood_val_dataset,  cfg.train.batch_size, shuffle=False, num_workers=cfg.num_workers)
    ood_test_loader = DataLoader(
        ood_test_dataset, cfg.train.batch_size, shuffle=False, num_workers=cfg.num_workers)
    eval_train_loader = DataLoader(
        eval_train_dataset, cfg.train.batch_size, shuffle=False, num_workers=cfg.num_workers)
    ...
    for run in range(cfg.train.runs):
        ...
        for epoch in range(cfg.train.epochs):
            ...
            test_perf, test_loss = test(test_loader, model,
                                      evaluator=evaluator, device=cfg.device)
            ood_val_perf, ood_val_loss = test(ood_val_loader, model,
                                      evaluator=evaluator, device=cfg.device)
            ood_test_perf, ood_test_loss = test(ood_test_loader, model,
                                      evaluator=evaluator, device=cfg.device)
            if val_perf >= best_val_perf:
                best_val_perf = val_perf
                test_perf, test_loss = test(test_loader, model,
            ...
            print("{epoch:03d}, Train: {train_perf:.4f}, Loss: {train_loss:.4f}, val: {val_perf:.4f}, test: {test_perf:.4f}, seconds: {time_cur_epoch:.4f}".format(epoch=epoch,
                train_perf=train_perf,
                train_loss=train_loss,
                val_perf=val_perf,
                test_perf=test_perf,
                time_cur_epoch=time_cur_epoch
                ),flush=True)
            print("{epoch:03d}, OOD Val: {ood_val_perf:.4f}, OOD Test: {ood_test_perf:.4f}".format(epoch=epoch,
                ood_val_perf=ood_val_perf,
                ood_test_perf=ood_test_perf
                ),flush=True)
            print(f'Epoch: {epoch:03d}, Train perf: {train_perf:.4f}, Train Loss: {train_loss:.4f}, '
                  f'Val: {val_perf:.4f}, Test: {test_perf:.4f}, Seconds: {time_cur_epoch:.4f}',flush=True)
            #   f'Memory Peak: {memory_allocated} MB allocated, {memory_reserved} MB reserved.')
